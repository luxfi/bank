...
const CheckboxContainer = styled(RadixCheckbox.Root)`
  ...
  background-color: ${({ checked, theme }) =>
    checked ? theme.backgroundColor.interactive['primary-default'].value : 'transparent'};
  border-width: ${({ theme, checked }) =>
    checked ? theme.borderWidth['width-zero'].value : theme.borderWidth['width-sm'].value};
