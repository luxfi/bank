...
(async function main() {
  ...
  export const variants: { [key in TTextVariants]: RuleSet<object> } = ${JSON.stringify(
    variantsStyles
  )
    .replace(/(\\n)/g, '')
    .replace(/("css)/g, 'css')
...
`;
  ...
  fs.writeFile('src/components/Text/constants.ts', formatted, (error) =>
    error ? console.log(error) : console.log('Successfully created Text types')
  );
})();
