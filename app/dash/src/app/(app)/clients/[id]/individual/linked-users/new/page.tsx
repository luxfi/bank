...
export default function AddLinkedUser() {
  const { postLinkedUser, clientSelected, loading, getClientsInfo } =
    useClients();
  ...
  const handleSubmit = async (values: ILinkedUserForm) => {
    ...
    await postLinkedUser(payload).then(() => {
      onShowMessage({
        ...
        description:
          'An email has been dispatched, and shortly he will receive a message with the link for their initial access.',
        status: 'success',
      });
