...
import { center, line, pill, stack } from '@/components/ui'
...
export function CardFace({ card, cvv, pan }: { card: CardView; cvv?: string; pan?: string }) {
  ...
  return (
    <View
      className={`card-face relative aspect-[1.586/1] w-full rounded-2xl overflow-hidden border border-white/10 p-5 transition-all ${
        frozen ? 'opacity-70' : ''
      }`}
      style={{ display: 'grid', gridTemplateRows: 'auto auto auto', alignContent: 'space-between' }}
    >
      {/* sheen */}
      <View
        className="-top-1/2 -right-1/4 w-[70%] h-[140%] rotate-12 bg-gradient-to-b from-white/10 to-transparent blur-2xl pointer-events-none"
        style={{ ...stack(), position: 'absolute' }}
      />

      <View className="relative" style={ends('start')}>
        <View className="text-white" style={line(8)}>
      ...
      <View className="relative" style={stack()}>
        {/* EMV chip — the gold pad with its contact plate, so it reads as a
            chip and not as a rectangle of gold. */}
        <View className="relative w-9 h-6 rounded-md bg-gradient-to-br from-yellow-200/80 to-yellow-500/60 mb-3" style={stack()}>
      ...
      <View className="relative" style={ends('end')}>
        <View style={stack()}>
      ...
      {frozen && (
        <View className="inset-0 bg-black/30 backdrop-blur-[1px]" style={{ ...center, position: 'absolute' }}>
          <span className="chip text-sky-200 border-sky-300/40 bg-black/40" style={pill('0.35rem')}>Frozen</span>
        </View>
      )}
    </View>
  )
