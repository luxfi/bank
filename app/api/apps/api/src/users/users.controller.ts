...
export class UsersController {
  ...
  async inviteFromAdmin(@Body() data: AdminInvitationDto) {
    ...
    if (
      ...
      !(await invitation.isSecretValid(data.code))
    ) {
    ...
    if (!user) throw new BadRequestException('The invitation is invalid.');
  ...
  @ApiResponse({
    status: 200,
    description: 'Return user with new current client.',
  })
  async selectCurrent(
    @Request() req,
  ...
  async updateCurrentUser(
    ...
    const userToSave = <User>(
      await this.usersRepository.findByUuid(req.user.uuid)
    );
    const exclude = data.afterRegistration ? true : false;
    if (exclude) {
      const pendingMetadata = await this.usersService.checkMetadataUpdate(
        userToSave,
        data,
      );
  ...
  async enable2FA(@Request() req, @Body() body) {
    const isCodeValid = await this.usersService.is2FAValid(
      req.user,
      body.token,
    );
    if (!isCodeValid) {
