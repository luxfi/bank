...
import { Invitation } from '@tools/models';

const heroImage = readFileSync(
  join(__dirname, 'templates/emails/registration/welcome/hero.gif'),
).toString('base64');

const helpCenterImage = readFileSync(
  join(__dirname, 'templates/emails/registration/welcome/help-centre-btn.gif'),
).toString('base64');

const icon1Image = readFileSync(
  join(__dirname, 'templates/emails/registration/welcome/icon-1.gif'),
).toString('base64');

const termsDocument = readFileSync(
  join(__dirname, 'templates/pdfs/terms_conditions.pdf'),
).toString('base64');
...
export default class AdminInvitationEmail extends BaseEmail<InvitationEmailContext> {
  constructor(invitation: Invitation, secret: string, password: string) {
    super(
      ...
      {
        invitationUrl: AdminInvitationEmail.getInvitationUrl(
          invitation,
          secret,
        ),
        password: password,
  ...
  private static getInvitationUrl(
    invitation: Invitation,
    secret: string,
  ): string {
    const url = new URL('/admin-invitation', process.env.FRONTEND_URL);
    url.search = `?invitation=${encodeURIComponent(
      invitation.uuid,
