...
import { snakeToWords } from '@tools/misc';
import {
  Account,
  AccountType,
  BankMetadataDto,
...
export class RegistrationService {
  ...
  /** Creates a new user with an account. */
  async createWithAccount(
    ...
    if (data.entityType === 'business') {
      ...
      // contact.setBusinessMetadata(businessMetadata);
    ...
    if (!skipWelcomeEmail) {
      await this.mailer.send(
        new WelcomeEmail(user.username, { fullName: user.getFullName() }),
      );
    }
  ...
  async validateBankMetadata(bankMetadata: BankMetadataDto, user: User) {
    ...
    if (
      !bankDetails ||
      bankDetails.errors ||
      bankDetails.bankCountry !== bankMetadata.bankCountry
    ) {
  ...
  async rejectMetadataTemp(
    ...
    await this.clientsmetadataTemp.removeAndFlush(temp);
  }
  ...
  async updateMetadata(
    ...
    // set expected transactions data.
    ...
    if (
      ...
      if (typeof data.businessMetadata !== 'undefined') {
        !!client &&
          (await client.setBusinessMetadata(
            this.normalizeBusinessMetadata(data.businessMetadata),
            exclude,
          ));
      }
    }
    ...
    if (data.userClientsMetadataDto) {
      ...
      const uc = await this.userClientsRepository.findByUserAndClientUuid(
        owner.uuid,
        client.uuid,
      );
      ...
      await this.userClientsRepository.persistAndFlush(uc);
    }
  ...
  private normalizeBusinessMetadata(
  ...
  private async sendMetadataEmail(
    user: User,
    data: UserMetadataDto,
    client: Client,
  ) {
    ...
    await this.clientsmetadataTemp.persistAndFlush(metadataTemp);
  ...
  async updateDocuments(user: User, data: UserDocumentsDto): Promise<User> {
    for (const dto of data.documents) {
      ...
      this.userDocumentsRepository.persist(userDocument);
    }

    await this.userDocumentsRepository.flush();
    return user;
  }
  ...
  private isWrongMetadataProvided(user: User, data: UserMetadataDto): boolean {
    ...
    return (
      ...
      (client.account.entityType === AccountType.Business &&
