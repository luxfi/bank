...
export class DocumentsManagerService {
  ...
  constructor(
    ...
    this.s3 = new AWS.S3({
      ...
      credentials: process.env.AWS_IS_LOCAL
  ...
  async uploadImage(user: User, file: Express.Multer.File): Promise<string> {
    try {
      ...
      const fileName = `image-${user.uuid}-${Date.now()}.${
        file.mimetype.split('/')[1]
      }`;

      this.logger.log(
        `:: uploadImage ${file.originalname} to ${bucketName} ::`,
      );
  ...
  async getDocumentFromBucket(key: string): Promise<S3Object> {
    try {
      ...
      throw new BadRequestException(
        `Failed to retrieve image from S3: ${error.message}`,
      );
    }
  }
  ...
  async upload(user: User, file: Express.Multer.File): Promise<Document> {
    ...
    try {
      ...
      await this.documentsRepository.persistAndFlush(document);
  ...
  async addDocumentToClient(
    ...
    try {
      if (type !== DocumentType.Other) {
        const exixtingDocumentType =
          await this.clientDocumentsRepository.findByDocumentType(client, type);
      ...
      await this.clientDocumentsRepository.persistAndFlush(clientDocument);
  ...
  async finalize(document: Document, type: DocumentType): Promise<Document> {
    ...
    // Define o novo caminho (chave) para o arquivo no S3
    const newPath = `finalized/${document.creator.username} (${
      document.creator.uuid
    })/${type} - ${new Date().toISOString()} - ${document.originalFilename}`;
    ...
    try {
      ...
      // Atualiza a chave do documento para o novo caminho
      ...
      this.logger.error(
        `Error finalizing document: ${error.message}`,
        error.stack,
      );
  ...
  async remove(document: Document) {
    ...
    try {
      ...
      // await this.documentsRepository.removeAndFlush(document);
    } catch (error) {
      this.logger.error(error.message, error.stack);
  ...
  async removeClientDocument(clientDocument: ClientDocument) {
    try {
      await this.clientDocumentsRepository.removeAndFlush(clientDocument);
    } catch (error) {
      this.logger.error(error.message, error.stack);
  ...
  async findByUuid(uuid: string): Promise<Document | null> {
    try {
      ...
      this.logger.error(
        `Error finding document by UUID: ${error.message}`,
        error.stack,
      );
  ...
  async findClientDocumentByDocument(
    document: Document,
  ): Promise<ClientDocument | null> {
    try {
      const clientDocument =
        await this.clientDocumentsRepository.findByDocument(document);
