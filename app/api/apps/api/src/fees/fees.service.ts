...
// import { OpenPaydService } from '../openpayd/open-payd.service';

import { Account, Fee, FeesRepository, User } from '@tools/models';
...
export class FeesService {
  ...
  constructor(
    private readonly feesRepository: FeesRepository, // @Inject(forwardRef(() => OpenPaydService)) // private readonly openPaydService: OpenPaydService,
  ) {}
  async create(data: any, user?: User | undefined): Promise<Fee | null> {
    const fee = await this.feesRepository.createFee(data, user);
    this.feesRepository.persistAndFlush(fee);
    return fee;
  }
  ...
  async getRawFeeAmount(amount, accountId, type, currency, payment_type = '') {
    ...
    const dbFee = await this.feesRepository.findOne({
      accountId,
    });
    if (dbFee) {
  ...
  async getFee(amount, accountId, type, currency, payment_type = '') {
    ...
    if (fee.type == 'percentage') {
      if (type == 'completed_conversion') {
        return Number(
          Number(amount - (amount / (100 + fee_amount)) * 100).toFixed(2),
        );
      } else {
