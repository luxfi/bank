...
import { OpenPaydCurrencies, User } from '@tools/models';
import axios from 'axios';
import { BeneficiariesService } from '../beneficiaries/services/beneficiaries.service';
import { FeesService } from '../fees/fees.service';
import { TransactionsService } from '../transactions/services/transactions.service';
import {
  CreateCurrencyBeneficiary,
  CreateCurrencyQuote,
  CreateCurrencyTopUp,
  CreateValidatePayment,
} from './model/create-currency-quote.dto';
...
import {
  CreateOpenPaydLinkedClientDto,
  UpdateOpenPaydLinkedClientDto,
} from './model/create-open-payd-linked-client';
...
export class OpenPaydService implements OpenPaydInterface {
  ...
  async getBasicRate(from: string, to: string) {
    ...
    //FIXME: add TTL to cache
    if (this.currencyRatesCache.get(currencyPair)) {
      return this.currencyRatesCache.get(currencyPair);
    }
    const response = await this.getRequest(
      `transactions/exchange/quote/basic?currencyPairs=${currencyPair}`,
    );
    if (response.status === 200) {
      this.currencyRatesCache = this.currencyRatesCache.set(
        currencyPair,
        response.data,
      );
  ...
  async createLinkedClient(
    contact: CreateOpenPaydLinkedClientDto,
  ): Promise<any> {
  ...
  async validateBeneficiaryToCreate(
    ...
    try {
      ...
      const validation = await this.postRequest(
        'beneficiaries/validate',
        {
          ...
          beneficiaryType:
            data.beneficiaryEntityType == 'individual' ? 'RETAIL' : 'CORPORATE',
          bankAccountType: data.bankAccountType,
          bankName: data.bankName,
          bankAddress: data.bankAddress,
          bankAccountHolderName:
            data.beneficiaryEntityType == 'individual'
  ...
  async createBeneficiary(
    ...
    const createBeneficiary = {
      beneficiaryType:
        beneficiary.beneficiaryEntityType == 'individual'
          ? 'RETAIL'
          : 'CORPORATE',
      friendlyName:
        beneficiary.beneficiaryEntityType == 'individual'
    ...
    if (response.status === 201) {
      ...
      await this.postRequest(
        `beneficiaries/${beneficiaryId}/bank-beneficiaries`,
        bankDetails,
        {},
        user,
      );
  ...
  async updateBeneficiary(
    ...
    if (response.status === 200) {
      ...
      const bankBeneficiaries = await this.getRequest(
        `bank-beneficiaries/?parentBeneficiaryIds=${id}`,
        {},
        user,
      );
      if (!bankBeneficiaries.data?.content?.length) {
        await this.postRequest(
          `beneficiaries/${response.data.id}/bank-beneficiaries`,
          bankDetails,
          {},
          user,
        );
        ...
        if (existingBeneficiaryBank.currency !== beneficiary.currency) {
          await this.deleteRequest(
            `bank-beneficiaries/${existingBeneficiaryBank.id}`,
            {},
            {},
            user,
          );
          ...
          await this.putRequest(
            `beneficiaries/bank-beneficiaries/${existingBeneficiaryBank.id}`,
            bankDetails,
            {},
            user,
          );
  ...
  async getBeneficiaries(currency: string, user: User) {
    try {
      const beneficiaries = (
        await this.beneficiariesService.getActiveBeneficiariesByCurrency(
          user,
          currency,
        )
      ).beneficiaries;
      const response = await this.getRequest(
        `bank-beneficiaries/?currencyIncluding=${currency}&size=100`,
        {},
        user,
      );
      if (response.status === 200) {
        return {
          beneficiaries: response.data.content
            .filter((beneficiary) => {
              return (
                beneficiaries.findIndex(
                  (beneficiaryFromDB) =>
                    beneficiaryFromDB.openPaydId == beneficiary.beneficiaryId,
                ) !== -1
              );
            })
            .map((beneficiary) => ({
              ...
              beneficiary_address: beneficiary.beneficiaryAddressLine
                ? [beneficiary.beneficiaryAddressLine]
                : [],
              ...
              beneficiary_entity_type:
                beneficiary.beneficiaryType == 'RETAIL'
                  ? 'individual'
                  : 'business',
              ...
              routing_code_type_1:
                beneficiary.bankRoutingCodes[0]?.routingCodeKey,
              routing_code_type_2:
                beneficiary.bankRoutingCodes[1]?.routingCodeKey || null,
              routing_code_value_1:
                beneficiary.bankRoutingCodes[0]?.routingCodeValue,
              routing_code_value_2:
                beneficiary.bankRoutingCodes[1]?.routingCodeValue || null,
              updated_at: '',
            })),
  ...
  async searchTransactions(
    ...
    try {
      ...
      if (shortReference) query += `&shortId=${shortReference}`;
      if (action) {
      ...
      if (amount_from) query += `&amountFrom=${amount_from}`;
      if (amount_to) query += `&amountTo=${amount_to}`;
      //if (scope) query += `&scope=${scope}`;
      if (page) query += `&page=${page}`;
      if (currency && !account_id) {
        const balances = await this.getBalances(user);
        const currencyAccount = balances?.balances
      ...
      if (response.status === 200) {
        const transactions = await Promise.all(
          response.data.content.map(async (t) => {
            ...
            const dbTX = await this.transactionsService.findByShortId(
              t.shortId,
              'openpayd',
            );
            const amountValue =
            ...
            const currencyValue =
              ...
              currency &&
              t.buyAmount.currency == currency
                ? t.buyAmount.currency
                : t.totalAmount.currency;
            const feeAmount =
              (t.type == 'EXCHANGE' &&
                ((currency && currency !== t.amount.currency) ||
                  t.fixedSide == 'BUY')) ||
              t.type === 'PAYOUT'
                ? Number(dbTX?.fee_amount || 0)
                : 0;
            const buyFeeAmount = t.buyAmount?.value
              ? Number(dbTX?.fee_amount || 0)
              : 0;
            ...
            return {
              ...
              buy_amount:
                t.buyAmount && t.buyAmount.currency !== currencyValue
                  ? (Math.abs(t.buyAmount?.value) - buyFeeAmount).toFixed(2)
                  : null,
              buy_currency:
                t.buyAmount && t.buyAmount.currency !== currencyValue
                  ? t.buyAmount?.currency
                  : null,
              sell_amount:
                t.sellAmount && t.sellAmount.currency !== currencyValue
                  ? Math.abs(t.sellAmount?.value).toFixed(2)
                  : null,
              sell_currency:
                t.sellAmount && t.sellAmount.currency !== currencyValue
                  ? t.sellAmount?.currency
                  : null,
              balance_amount: null,
      ...
      this.logger.error('Something went wrong', err.message, err.stack);
    }
    return null;
  }
  async getTransaction(
    ...
    try {
      //FIXME
      const response = await this.getRequest(
        `transactions?currency=${currency}&page=${page - 1}`,
        {},
        user,
      );
      if (response.status === 200) {
  ...
  async getPayinDetails(transaction_id: string, user: User | null) {
    const response = await this.getRequest(
      `transactions/payin/${transaction_id}`,
      {},
      user,
    );
    return response.data;
  ...
  async getConversion(uuid: string, user: User) {
    try {
      ...
      if (response.status === 200) {
        ...
        const amountValue =
          t.type == 'EXCHANGE' && currency && t.buyAmount.currency == currency
            ? t.buyAmount.value
            : t.amount.value;
        const currencyValue =
          t.type == 'EXCHANGE' && currency && t.buyAmount.currency == currency
        ...
        const fxRate = Number(Number(buyAmount) / Number(sellAmount)).toFixed(
          5,
        );
        return {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        return {
          ...
          type:
            Number(amountValue) > 0 && related_entity_type != 'inbound_funds'
              ? 'debit'
              : 'credit',
          updated_at: t.updatedDate,
          //FIXME: get fee from the DB,
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        return {
          ...
          beneficiary_date_of_birth: null,
          beneficiary_entity_type:
            beneficiary.beneficiaryType == 'RETAIL' ? 'individual' : 'business',
          beneficiary_external_reference: null,
          beneficiary_first_name: beneficiary.beneficiaryFirstName,
          beneficiary_identification_type: null,
          beneficiary_identification_value: null,
  ...
  async getCurrentAccount(user: User) {
    ...
    const currentAccount = clients.filter(
      (c) => c.accountHolderId == user.contact?.account.openPaydId,
    )[0];
  ...
  async getSettleAccounts(currency: string, account: string, user: User) {
    try {
      const response = await this.getRequest(
        `bank-accounts?currency=${currency}&status=ACTIVE`,
        {},
        user,
      );
      if (response.status === 200) {
        return {
          settlement_accounts: response.data.map((account) => ({
            ...
            iban: account.iban,
            routing_code_type_1: account.routingCodeEntries?.length
              ? account.routingCodeEntries[0].routingCodeKey
              : '',
            routing_code_value_1: account.routingCodeEntries?.length
              ? account.routingCodeEntries[0].routingCodeValue
              : '',
    ...
    try {
      ...
      const buy_currency_account =
        balances[
          balances.findIndex((balance) =>
            buy_currency.length == 3
              ? balance.currency == buy_currency
              : balance.id == buy_currency,
          )
        ].account_id;
      const sell_currency_account =
        balances[
          balances.findIndex((balance) =>
            sell_currency.length == 3
              ? balance.currency == sell_currency
              : balance.id == sell_currency,
          )
        ].account_id;
      ...
      const conversion_amount =
        fixed_side == 'sell'
          ...
          : Number(
              Number(amount) +
                (Number(amount) * Number(rawFeeAmount.amount)) / 100,
            ).toFixed(2);
      const response = await this.postRequest(
      ...
      if (response.status === 200) {
        ...
        const buyAmount = Number(
          fixed_side == 'sell'
            ? Number(
                (response.data.buyAmount.value * (100 - rawFeeAmount.amount)) /
                  100,
              )
            : amount,
        ).toFixed(2);
    ...
    try {
      ...
      const buy_currency_account =
        balances[
          balances.findIndex((balance) =>
            data.buy_account_id
              ? balance.id == data.buy_account_id
              : balance.currency == data.buy_currency,
          )
        ].account_id;
      const sell_currency_account =
        balances[
          balances.findIndex((balance) =>
            data.sell_account_id
              ? balance.id == data.sell_account_id
              : balance.currency == data.sell_currency,
          )
        ].account_id;
      const response = await this.postRequest(
      ...
      if (response.status === 201) {
        ...
        //Complete the conversion on staging automatically
        ...
        const amountForFee =
          data.fixed_side == 'sell'
            ? quoteData.buyAmount.value
            : (quoteData.buyAmount.value / (rawFee.amount + 100)) * 100;
        ...
        const buyAmount = Number(quoteData.buyAmount.value - feeAmount).toFixed(
          2,
        );
        const clientFxRate = Number(
          Number(buyAmount) / Number(quoteData.sellAmount.value),
        ).toFixed(5);
        ...
        return {
          ...
          client_buy_amount: Number(
            quoteData.buyAmount.value - feeAmount,
          ).toFixed(2),
          client_rate: clientFxRate,
  ...
  async getFees(data: CreateValidatePayment, payment_type: string, user: User) {
    try {
      ...
      if (response.status === 200) {
        ...
        return {
          fee_amount: Number(
            Number(payment.feeAmount.value) + feeAmount,
          ).toFixed(2),
  ...
  async createPayment(data: CreateValidatePayment, user: User) {
    try {
      ...
      if (response.status === 201) {
        ...
        //Complete the payment on staging automatically
        if (process.env.OPENPAYD_LOGIN == 'michailpayd') {
  ...
  async getMainBalance(currency: string) {
    ...
    const mainBalance = mainBalances?.balances
      ?.filter((b) => b.currency == currency)
      .map((b) => b.account_id);
    if (!mainBalance.length) {
      ...
      //FIXME -- use separate 'fees' account ?
  ...
  async getFeeAmount(
    ...
    try {
      //FIXME: move the fee amount to DB
      const currency =
        data instanceof CreateValidatePayment
          ? data.currency
          : data.sell_currency;
      const type =
        data instanceof CreateValidatePayment ? 'payment' : 'conversion';
      const feeAmount = await this.feesService.getFee(
        data.amount,
        account_id,
        type,
  ...
  async renameLinkedClient(user: User, id: string, friendlyName: string) {
    const rename = await this.putRequest(
      `linkedClient/${id}`,
      { friendlyName },
      {},
      user,
  ...
  async deleteBeneficiary(id: string, user: User): Promise<void> {
    const deleteRequest = await this.deleteRequest(
      `beneficiaries/${id}`,
      {},
      {},
      user,
  ...
  private isTokenExpired() {
    return (
      !this.tokenCreatedAt ||
      new Date().valueOf() - this.tokenCreatedAt.valueOf() >
        1000 * (Number(this.tokenExpiresIn) - 3)
    );
  }
  ...
  private removeValidateBeneficiaryExtraParameters(
    beneficiary: CreateOpenPaydBeneficiaryDto,
  ...
  private async deleteRequest(
    ...
    const baseUrl =
      process.env.OPENPAYD_BASE_URL || 'https://sandbox.openpayd.com/api';
