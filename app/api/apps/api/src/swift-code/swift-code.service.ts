...
import { Beneficiary, CreateBeneficiaryDto, User } from '@tools/models';
...
export class SwiftCodeService {
  ...
  async getDetailsByAccountNumberAndSortCode(
    accountNumber: string,
    sortCode: string,
  ) {
    try {
      const iban = 'GB33BUKB' + sortCode + accountNumber;
      const ibanResponse = await this.request<IbanResponseDto>(
        `v1/ibans/${iban}`,
      );
  ...
  async getDetailsBySwift(swift: string) {
    try {
      const swiftResponse = await this.request<SwiftResponseDto>(
        `v1/swifts/${swift}`,
      );
  ...
  private async request<T>(endpoint: string): Promise<AxiosResponse<T>> {
    ...
    return axios.get<T, AxiosResponse<T>>(`${baseUrl}/${endpoint}`, {
      headers: {
        ...
        'X-Api-Key': String(process.env.SWIFT_CODE_API_KEY),
      },
    });
  }

  async getBankDetails(
    createBeneficiaryDto: CreateBeneficiaryDto | Beneficiary,
    user: User,
  ) {
    let details;
    if (createBeneficiaryDto.IBAN) {
      ...
      if (createBeneficiaryDto.bicSwift) {
        const detailsVerification = await this.getDetailsBySwift(
          createBeneficiaryDto.bicSwift,
        );
    ...
    if (
      !details &&
      isInUk(createBeneficiaryDto.bankCountry) &&
      createBeneficiaryDto.accountNumber !== '' &&
      createBeneficiaryDto.sortCode !== ''
    ) {
      details = await this.getDetailsByAccountNumberAndSortCode(
        createBeneficiaryDto.accountNumber,
        createBeneficiaryDto.sortCode,
      );
    }
    if (details !== null && typeof details !== 'undefined') {
      ...
      const currencyCloudData =
        createCurrencyCloudBeneficiaryDto(createBeneficiaryDto);
      const { errors: currencyCloudErrors, beneficiary } =
        await this.validateOnCurrencyCloud(currencyCloudData, '', user);
      return beneficiary
        ? {
  ...
  async validateOnCurrencyCloud(
    data: CreateCurrencyCloudBeneficiaryDto,
    updaterId: string,
    user: User,
  ): Promise<any> {
    ...
    const validation =
      await this.currencyCloudService.validateBeneficiaryToCreate(
        data,
        updaterId,
        user,
      );
