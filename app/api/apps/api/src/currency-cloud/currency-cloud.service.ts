...
import { getConfigCurrencyCloud } from '@tools/misc/utils/clientSetting';
...
export class CurrencyCloudService implements CurrencyCloudInterface {
  ...
  async getPayerAndBankInfo(clientUuid: string): Promise<{
    ...
    const payer =
      client?.account?.individualMetadata || client?.account?.businessMetadata;
  ...
  async getBalances(user: User) {
    try {
      ...
      if (response.status === 200) {
        return {
          balances: response.data.balances.map((d) => ({
            ...d,
            id: d.currency + d.id,
          })),
        };
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        return {
          ...
          beneficiaries: response.data.beneficiaries.filter((beneficiary) => {
            return (
              beneficiaries.length &&
              beneficiaries.findIndex(
                (beneficiaryFromDB) =>
                  beneficiaryFromDB.currencyCloudId == beneficiary.id,
              ) !== -1
            );
          }),
        };
  ...
  async findDashboardInfo(
    ...
    try {
      ...
      if (response.status === 200) {
        return response.data.pagination.total_entries;
      }
    } catch (err) {
      this.logger.error(err.message, err.stack);
    }
  ...
  async searchTransactions(
    ...
    try {
      ...
      if (settles_at_from) params['settles_at_from'] = settles_at_from;
      if (settles_at_to) params['settles_at_to'] = settles_at_to;
      if (created_at_from) params['created_at_from'] = created_at_from;
      if (created_at_to) params['created_at_to'] = created_at_to;
      if (completed_at_from) params['completed_at_from'] = completed_at_from;
      if (completed_at_to) params['completed_at_to'] = completed_at_to;
      if (status) params['status'] = status;
      if (type) params['type'] = type;
      if (amount_from) params['amount_from'] = amount_from;
      if (amount_to) params['amount_to'] = amount_to;
    ...
    try {
      ...
      return {
        subaccounts: subaccounts.map((contact) => {
          const contactAccount = accounts.find(
            (a) => a.id == contact.account_id,
          );
          const disabled = contactAccount?.status !== 'enabled';
          return {
            accountHolderId: contact.id,
            clientType: contact.login_id,
            friendlyName: `${disabled ? 'DISABLED - ' : ''} ${
              contactAccount?.account_name
            } (${contact.first_name} ${contact.last_name})`,
            taken:
              existingSubaccountIds?.findIndex(
                (existing) => existing == contact.id,
              ) !== -1,
          };
        }),
      };
  ...
  async getSettleAccounts(currency: string, account_id: string, user: User) {
    try {
      ...
      if (
        responseSettlementAccounts.status === 200 &&
        responseFundingAccounts.status === 200
      ) {
        return {
          ...responseSettlementAccounts.data,
          ...responseFundingAccounts.data,
        };
      }
    } catch (err) {
      this.logger.error(err.message, err.stack);
  ...
  async getCurrencyQuote(
    ...
    try {
      ...
      if (response.status === 200) {
        return { ...response.data, ...conversionDates };
      }
    } catch (err) {
      this.logger.error(err.message, err.stack);
    }
  ...
  private async logout(baseUrl: string): Promise<void> {
    ...
    try {
      ...
      await axios.post(`${baseUrl}/authenticate/close_session`, null, {
        headers,
      });
    } catch (err) {
  ...
  private async postRequest(
    ...
    if (isOnBehalfOf && user.getCurrentClient()) {
      extraData.on_behalf_of =
        user.getCurrentClient()?.account?.cloudCurrencyId;
    }

    const { baseUrl, apiKey, loginId } = getConfigCurrencyCloud(
    ...
    try {
      ...
      this.logger.error(uri, error.message, error.stack);
      return error;
    }
  }
