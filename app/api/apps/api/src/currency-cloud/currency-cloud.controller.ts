...
export class CurrencyCloudController {
  ...
  async beneficiaries(@Param('currency') currency: string, @Request() req) {
    const data = await this.currencyCloudService.getBeneficiaries(
      currency,
      req.user,
    );
    return new SuccessResponse(data);
  ...
  async transactionref(@Param('short_ref') short_ref: string, @Request() req) {
    const data = await this.currencyCloudService.getTransactionRef(
      short_ref,
      req.user,
    );
    return new SuccessResponse(data);
  ...
  async conversion(@Param('uuid') uuid: string, @Request() req) {
    ...
    return new SuccessResponse({
      ...data,
      status: TransactionStatusLabel[data.status],
    });
  }
  ...
  async dateChange(
    ...
    @Request() req,
    @Body() { new_settlement_date }: DateChangeDTO,
  ) {
    const data = await this.currencyCloudService.dateChange(
      uuid,
      new_settlement_date,
      req.user,
    );
  ...
  async splitPreview(
    @Param('uuid') uuid: string,
    @Request() req,
    @Body() { amount }: SplitConversionDTO,
  ) {
    const data = await this.currencyCloudService.splitPreview(
      uuid,
      amount,
      req.user,
    );
  ...
  async payment(@Param('uuid') uuid: string, @Request() req) {
    ...
    const payment = await this.transactionsService.findByTransactionId(
      uuid,
      'currencycloud',
    );
    ...
    return new SuccessResponse({
      ...data,
      status: dayjs(dayjs(payment.payment_date).format('YYYY-MM-DD')).isAfter(
        dayjs(),
      )
  ...
  async currencyaccount(@Param('currency') currency: string, @Request() req) {
    const data = await this.currencyCloudService.createCurrencyAcccount(
      currency,
      req.user,
    );
    return new SuccessResponse(data);
  ...
  async account(@Param('account_id') account_id: string, @Request() req) {
    const data = await this.currencyCloudService.getAccount(
      account_id,
      req.user,
    );
    return new SuccessResponse(data);
  ...
  async contacts(
    ...
    const data = await this.currencyCloudService.getContacts(
      account_id,
      req.user,
    );
    return new SuccessResponse(data);
  ...
  async all_contacts(
    ...
    const data = await this.currencyCloudService.getAllEnabledContacts(
      req.user,
    );
    return new SuccessResponse(data);
  }
  ...
  async contact(@Param('contact_id') contact_id: string, @Request() req) {
    const data = await this.currencyCloudService.getContact(
      contact_id,
      req.user,
    );
    return new SuccessResponse(data);
  ...
  @Roles(...ManagerRoles, UserRole.TeamMember, UserRole.ViewerUser)
  async settle_accounts(
    @Param('currency') currency: string,
    @Param('account_id') account_id: string,
    @Request() req,
  ) {
    const data = await this.currencyCloudService.getSettleAccounts(
      currency,
      account_id,
      req.user,
    );
  ...
  @UsePipes(new ValidationPipe({ transform: true }))
  async create_quote(
    @Body() createCurrencyQuoteDto: CreateCurrencyQuote,
    @Request() req,
  ) {
    const quote = await this.currencyCloudService.postCurrencyQuote(
      createCurrencyQuoteDto,
      req.user,
    );
  ...
  @UsePipes(new ValidationPipe({ transform: true }))
  async create_top_up(
    @Body() createCurrencyTopUpDto: CreateCurrencyTopUp,
    @Request() req,
  ) {
    const top_up = await this.currencyCloudService.postCurrencyTopUp(
      createCurrencyTopUpDto,
      req.user,
    );
  ...
  async validate_payment(
    @Body() createValidatePaymentDto: CreateValidatePayment,
    @Request() req,
  ) {
    const payment = await this.currencyCloudService.validatePayment(
      createValidatePaymentDto,
      req.user,
    );
  ...
  async create_payment(
    ...
    const status_approval = MemberRoles.includes(req.user.role)
    ...
    const beneficiary = await this.currencyCloudService.getBeneficiary(
      createValidatePaymentDto.beneficiary_id,
      req.user,
    );

    const payerAndBankInfo =
      await this.currencyCloudService.getPayerAndBankInfo(
        paymentTransaction.client_uuid,
      );
    ...
    if (status_approval === TransactionStatusApproval.Done) {
      ...
      if (response) {
        ...
        throw new BadRequestException({
          message: 'Payment could not be created on currencycloud',
        });
      }
    } else {
  ...
  async edit_payment(
    ...
    const payment = await this.currencyCloudService.editPayment(
      createValidatePaymentDto,
      req.user,
    );
    return new SuccessResponse({ payment });
  ...
  async delete_payment(@Body() uuid: DeletePaymentDTO, @Request() req) {
    const payment = await this.currencyCloudService.deletePayment(
      uuid.uuid,
      req.user,
    );
    return new SuccessResponse({ payment });
  ...
  async cancel_conversion(@Body() uuid: DeletePaymentDTO, @Request() req) {
    const payment = await this.currencyCloudService.cancelConversion(
      uuid.uuid,
      req.user,
    );
    return new SuccessResponse({ payment });
  ...
  async create_beneficiary(
    @Body() createCurrencyBeneficiaryDto: CreateCurrencyBeneficiary,
    @Request() req,
  ) {
    const top_up = await this.currencyCloudService.postCurrencyBeneficiary(
      createCurrencyBeneficiaryDto,
      req.user,
    );
    return new SuccessResponse({ top_up });
  }
  ...
  @Post('delivery_date')
  @Roles(...UserRoles)
  async load_delivery_date(
    @Body() deliveryDateDTO: DeliveryDateDTO,
    @Request() req,
  ) {
  ...
  @Roles(...UserRoles)
  async conversion_dates(
    @Param('pair') conversion_pair: string,
    @Request() req,
  ) {
    const data = this.currencyCloudService.getConversionDates(
      conversion_pair,
      req.user,
    );
    return new SuccessResponse(data);
  ...
  async approve_payment(
    @Param('uuid') uuid: string,
    @Body() body: TransactionDescriptionDto,
    @Request() req,
  ) {
    const accountId = req.user.getCurrentClient().account.cloudCurrencyId;
    const payment = await this.transactionsService.findByUuidAndAccountId(
      uuid,
      accountId,
    );
    ...
    if (response) {
      ...
      throw new BadRequestException({
        message: 'Payment could not be created on currency cloud',
      });
    }
  }
  ...
  async reject_payment(
    ...
    return new SuccessResponse({
      status_approval: TransactionStatusApproval.Rejected,
    });
  }
  ...
  async findPending(@Request() req, @Param('uuid') uuid: string) {
    ...
    const transaction = await this.transactionsService.findByUuidAndAccountId(
      uuid,
      accountId,
    );
