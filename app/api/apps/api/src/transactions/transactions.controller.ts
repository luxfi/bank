...
export class TransactionsController {
  ...
  async searchTransactions(
    ...
    const [transactions, totalElements] =
      await this.transactionsService.searchTransactions(user, query);
    const pageSize = 20;
    ...
    return new SuccessResponse({
      transactions: transactions.map((transaction) => {
        return {
          ...
          buy_amount:
            transaction.action === 'conversion' ? transaction.buy_amount : '',
          buy_currency:
            transaction.action === 'conversion' ? transaction.buy_currency : '',
          sell_amount:
            transaction.action === 'conversion' ? transaction.amount : '',
          sell_currency:
            transaction.action === 'conversion' ? transaction.currency : '',
          balance_amount: null,
