...
import { CurrencyCloudConversion } from '@tools/models/transactions/dtos/create-conversion.dto';
...
export class TransactionsService {
  ...
  async searchTransactions(
    ...
    if (status_approval === TransactionStatus.Expired) {
      ...
      completed_at_from =
        completed_at_from ||
        dayjs().subtract(100, 'years').format('YYYY-MM-DD');
    }
    ...
    if (status_approval) {
      search.$and.push({
        status_approval: { $in: status_approval.split(',') },
      });
    }
    if (reference) {
      search.$and.push({
    ...
    if (!!user && !!user.getCurrentClient()) {
      ...
      if (account) {
        search.$and.push({
          account_id: account.gatewayId || account.cloudCurrencyId,
        });
      }
    }
