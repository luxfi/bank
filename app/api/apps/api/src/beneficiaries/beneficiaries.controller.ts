...
import { CreateBeneficiaryDto, UserRole, ManagerRoles } from '@tools/models';
...
export class BeneficiariesController {
  ...
  async create(
    ...
    const { beneficiary, errors } = await this.beneficiariesService.create(
  ...
  async list(@Request() req, @Query() query: FilterBeneficiariesDTO) {
    const data = await this.beneficiariesService.getActiveBeneficiaries(
      req.user,
      query,
    );
  ...
  async fetch(@Param('uuid') uuid: string, @Request() req) {
    const beneficiary = await this.beneficiariesService.getOneForUser(
      req.user,
      uuid,
    );
    if (beneficiary) {
  ...
  async update(
    ...
    const beneficiaryDB = await this.beneficiariesService.getOneForUser(
      req.user,
      uuid,
    );
    if (beneficiaryDB) {
  ...
  async approve(@Param('uuid') uuid: string, @Request() req) {
    ...
    if (beneficiary) {
      const updatedBeneficiary =
        await this.beneficiariesService.approveBeneficiary(
          beneficiary,
          req.user,
        );
  ...
  async listByClient(
    ...
    @Query() query: FilterBeneficiariesAccountDTO,
  ) {
    ...
    const data =
      await this.beneficiariesService.getActiveBeneficiariesByAccount(
        client.account.uuid,
        query,
      );
