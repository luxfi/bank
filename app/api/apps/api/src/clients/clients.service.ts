...
import {
  ClientSelectPaginatedResponse,
  ClientsRepository,
} from '@tools/models/clients/repository';
...
export class ClientsService {
  ...
  async createClient(
    ...
    await this.clientsRepository.persistAndFlush(client);
    return client;
  }

  async putUserOnClient(
    user: User,
    client: Client,
    role: UserRole,
    phoneNumber = '',
    ...
    await this.userClientsRepository.persistAndFlush(userClient);
    return userClient;
  }
  ...
  async getMetadata(
    ...
    return this.userClientsRepository.findByUserAndClientUuid(
      userUUID,
      clientUUID,
    );
  }
  ...
  async getAllClients(
    ...
    return this.clientsRepository.findAndCountNonSubAccount(
      name,
      entityType,
      country,
      page,
  ...
  async updateClient(
    ...
    await this.clientsRepository.flush();
    return client;
  }
  ...
  async activeDeactivedClient(uuid: string): Promise<boolean> {
    ...
    await this.clientsRepository.assign(client, {
      deletedAt: null,
      updatedAt: new Date(),
    });
    await this.clientsRepository.flush();
