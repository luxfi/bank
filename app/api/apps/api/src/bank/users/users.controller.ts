...
import { GetUser, SuccessResponse, SuccessResponseV2 } from '@tools/misc';
import { AdminRoles, ManagerRoles, User, UserRoles } from '@tools/models';
import { Roles } from '../../auth/roles.decorator';
import {
  GetRequestDto,
  GetUsersRequestDto,
  GetUsersSelectRequestDto,
} from './dto/get-users.request.dto';
...
export class UsersController {
  ...
  async listSelect(
    @Query() query: GetUsersSelectRequestDto,
    @GetUser() user: User,
  ): Promise<ListUsersSelectResponseDto> {
    const { data, pagination } =
      await this.getUsersBySelectDomainUseCase.handle(query, user);
  ...
  async list(
    ...
    const { data, pagination } = await this.getUsersDomainUseCase.handle(
      query,
      user,
    );
    return new SuccessResponseV2(data, pagination);
  ...
  async fetch(
    ...
    const result = await this.getUserDomainUseCase.handle(
      { id, clientId: query.clientId },
      user,
    );
  ...
  async update(
    ...
    const result = await this.updateUserDomainUseCase.handle(
      { id, ...data },
      user,
    );
    return new SuccessResponseV2(result);
