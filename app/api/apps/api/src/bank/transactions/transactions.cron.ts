  ...
  async handleCron() {
    ...
    for (const transaction of pendingTransactions) {
      try {
        ...
        if (!creator) {
          this.logger.error(
            `:: Creator not found: ${transaction.creator_uuid} ::`,
          );
          continue;
        }
        ...
        if (transaction.action === 'payment') {
          const response = await ifx.getPaymentDetail(
          ...
          if (
            ...
            transaction.settlement_date = dayjs(response.arriveBy).format(
              'YYYY-MM-DD',
            );
            await this.transactionsRepository.persistAndFlush(transaction);

            this.logger.log(
              `:: Processed Payment: ${transaction.uuid} ::`,
              response.status,
            );
          }
        }
        ...
        if (transaction.action === 'conversion') {
          const response = await ifx.getConversionDetail(
            transaction.transaction_id,
          );

          this.logger.log(
            `:: Conversion: ${transaction.uuid} ::`,
            response.dueDate,
          );
          ...
          if (dayjs(response.dueDate).isBefore(dayjs())) {
            ...
            await this.transactionsRepository.persistAndFlush(transaction);
            this.logger.log(
              `:: Processed Conversion: ${transaction.uuid} ::`,
              response.dueDate,
            );
          }
        }
        ...
        this.logger.error(
          `:: Error processing ${transaction.uuid}:`,
          error.message,
        );
      }
    }
