import { GetTotalTransactionsDomainUseCase } from '@domain/use-cases';
...
import { User, UserRole, UserRoles } from '@tools/models';
...
export class TransactionsController {
  ...
  async findAll(
    ...
    const { data, pagination } = await this.getTransactionsUseCase.handle(
      query,
      user,
    );
  ...
  async getTotalTransactions(
    @GetUser() user: User,
  ): Promise<GetTransactionTotalResponseDto> {
  ...
  async findOne(
    @Param('id') id: string,
    @GetUser() user: User,
  ): Promise<GetTransactionResponseWithDataDto> {
