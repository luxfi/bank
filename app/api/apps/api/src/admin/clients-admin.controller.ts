...
import { AxiosOAuthClient } from '@ports/ifx/oauth-client';
import { GetUser, SuccessResponse } from '@tools/misc';
import {
  AdminClientDto,
  AdminCreateUserDto,
  AdminResetPasswordDto,
  AdminRoles,
  CurrencyCloudDataDto,
  IfxLinkDataDto,
  ManagerRoles,
  RiskAssessmentDto,
  User,
  UserMetadataDto,
  UserMetadataRejectDto,
  UserRole,
} from '@tools/models';
...
export class ClientsAdminController {
  ...
  @Get('')
  async list(
    @Query(new ValidationPipe({ transform: true })) query: SearchUsersDto,
  ) {
  ...
  async create(
    ...
    const client = await this.usersAdminService.createUser(
      data,
      ip,
      UserRole.AdminUser,
    );
  ...
  async updateMetadata(
    ...
    const client = await this.usersAdminService.updateMetadataTemp(
      uuid,
      data,
      user,
    );
  ...
  @UsePipes(new ValidationPipe({ transform: true }))
  async updateMetadataTemp(
    @Param('uuid') uuid: string,
    @Body() data: UserMetadataDto,
    @GetUser() user: User,
  ) {
    const client = await this.usersAdminService.updateMetadataTempwithoutsave(
      uuid,
      data,
      user,
    );
  ...
  @ApiResponse({ status: 404, description: 'Client not found.' })
  async approveTempChange(
    @Param('uuid') uuid: string,
    @Param('tempId') tempId: string,
    @GetUser() user: User,
  ): Promise<any> {
    const data = await this.usersAdminService.approveMetadata(
      uuid,
      tempId,
      user,
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'The temporary change data has been successfully rejected.',
  })
  @ApiResponse({ status: 404, description: 'Client not found.' })
  async rejectTempChange(
    @Param('uuid') uuid: string,
    @Param('tempId') tempId: string,
    @Body() data: UserMetadataRejectDto,
    @GetUser() user: User,
  ): Promise<any> {
    const result = await this.usersAdminService.rejectMetadata(
      uuid,
      tempId,
      data,
      user,
  ...
  @UsePipes(new ValidationPipe({ transform: true }))
  async updateCurrencyCloudData(
    @Param('uuid') uuid: string,
    @Body() data: CurrencyCloudDataDto,
  ) {
    const client = await this.usersAdminService.updateCurrencyCloudData(
      uuid,
      data,
    );
    return new SuccessResponse({ client });
  ...
  async approveDocument(
    ...
    if (data.data) return new SuccessResponse({ document: data.data });
    else if (data.error) throw new NotFoundException(data.error);
  ...
  ): Promise<SuccessResponse<AdminClientDto>> {
    const client = await this.clientService.findByUuidWithRiskAssessments(
      clientUuid,
    );
  ...
  async createRiskAssessment(
    @Param('clientUuid') clientUuid: string,
    @Body() data: RiskAssessmentDto,
  ) {
    const newRiskAssessment = await this.clientService.saveNewRiskAssessment(
      clientUuid,
      data,
    );
