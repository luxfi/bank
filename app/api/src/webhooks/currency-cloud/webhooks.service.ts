...
export class WebhooksCurrencyCloudService {
    ...
    async updateOrCreateConversionFromWebhook(data: CurrencyCloudDTO) {
        ...
        if (tx && TransactionStatusMap['completed'].includes(tx.status as TransactionStatus))
            return;

        if (!tx) {
            const client = await this.clientsRepository.findOne({account: {cloudCurrencyId: data.creator_contact_id}});
            if (!client)
            ...
            tx.status = data.status;
            tx.settlement_date = data.settlement_date;
            if (TransactionStatusLabel[data.status] === 'completed' || TransactionStatusLabel[data.status] === 'deleted')
                tx.gateway_completed_at = new Date().toISOString();
        }

        await this.transactionsRepository.getEntityManager().persistAndFlush(tx);
        return tx;
    }
    ...
    async updateOrCreatePaymentFromWebhook(data: CurrencyCloudDTO) {
        ...
        let tx = await this.transactionsRepository.findOne({transaction_id: data.id, action: TransactionAction.Payment});
        if (tx && TransactionStatusMap['completed'].includes(tx.status as TransactionStatus))
            return;
    ...
    async updateOrCreateFundingFromWebhook(data: CurrencyCloudDTO) {
        ...
        let tx = await this.transactionsRepository.findOne({transaction_id: data.id, action: TransactionAction.Funding});
        if (tx && TransactionStatusMap['completed'].includes(tx.status as TransactionStatus))
            return;
