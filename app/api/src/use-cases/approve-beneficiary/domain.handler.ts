...
export class ApproveBeneficiaryDomainUseCase {
    ...
    async handle(query: ApproveBeneficiaryRequest, user: User): Promise<ApproveBeneficiaryResponse> {
        const beneficiary = await this.beneficiariesRepository.findOneByUserAndId(user, query.id);

        if (!beneficiary)
            throw new BadRequestException({messages: ['Beneficiary not found']});

        if (beneficiary.gatewayId)
            throw new BadRequestException({messages: ['Beneficiary already approved']});
    ...
    private async createOnCurrencyCloud(beneficiary: Beneficiary) {
        ...
        if (!cloudCurrencyId)
            throw new BadRequestException({messages: ['Cloud currency not found']});
        ...
        const result = await gateway.createBeneficiary(currencyCloudData, beneficiary.creator);

        if (!result)
            throw new BadRequestException({messages: ['Error creating beneficiary']});
