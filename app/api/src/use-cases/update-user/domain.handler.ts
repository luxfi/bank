...
export class UpdateUserDomainUseCase {
    ...
    async handle(query: UpdateUserRequest, user: User): Promise<UserUpdateResponse> {
        const { id } = query;
        ...
        if (_user.role !== UserRole.SuperAdmin && !!query.password)
            throw new BadRequestException({messages: ['Only SuperAdmin can update password.']});
    ...
    async updateUsers(data: UpdateUserRequest, uuid?: string) {
        ...
        if (!user)
            throw new NotFoundException({messages: ['User not found.']});
    ...
    async updateRoleMetadata(userUUID: string, clientUUID: string, role: UserRole): Promise<UserClient> {
        ...
        if (!userClient)
            throw new NotFoundException({messages: ['UserClient not found.']});
