...
export class GetBeneficiaryDetailDomainUseCase {
    ...
    async handle(param: GetBeneficiaryDetailRequest, user: User): Promise<GetBeneficiaryDetailResponse> {
        const beneficiary = await this.beneficiariesRepository.findOneByUserAndId(user, param.id);
