...
export class MobileVerificationRepository extends EntityRepository<MobileVerificationEntity> {
    ...
    async findRecentlyVerified(mobileNumber: string, code: string, ip: string): Promise<MobileVerificationEntity | null> {
        return this.findOne({
            mobileNumber,
            code,
            ip,
    ...
    async findRecentlyVerifiedByIP(mobileNumber: string, ip: string): Promise<MobileVerificationEntity | null> {
        return this.findOne({
            mobileNumber,
            ip,
            createdAt: {
