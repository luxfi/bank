...
@Entity({ customRepository: () => ClientsRepository })
export class Client extends BaseEntity {
  ...
  getMetadataByUser(uuid: string) {
    return this.userClients.getItems().find((uc) => uc.user.uuid === uuid)
      ?.metadata;
  }
  ...
  getOwner(): User | undefined {
    return this.userClients
      ?.getItems()
      .find((uc) => uc.metadata.role === UserRole.AdminUser)?.user;
  }
  ...
  constructor(data?: any) {
    ...
    //this.users = data?.users ?? new Collection<User>(this);
  }
