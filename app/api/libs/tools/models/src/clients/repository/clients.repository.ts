...
import { RiskAssessment, UserRole, UserRoles } from '@tools/models/users';
...
export class ClientsRepository extends EntityRepository<Client> {
  ...
  async findAllWithRiskAssessmentsPaginated(
    params: GetRiskClientsRequest,
    ...
    const data = Array.from(clients).flatMap((client) => {
      const recentRisk = client.account?.riskAssessments[0]; // Assuming the most recent risk assessment is at index 0 after sort
  ...
  async findAllWithRiskAssessments({
    page = 1,
    limit = 10,
  }): Promise<ClientSelectRiskPaginatedResponse> {
    const query = <any>{
    ...
    return {
      data: Array.from(clients).map((client) => {
        const currentRisk =
          client.account?.getRecentRiskAssessment() ?? new RiskAssessment();
        return {
          id: client.uuid,
          type: client.account?.entityType ?? '',
          name: client.getAccountName() ?? '',
  ...
  async findAndCountNonSubAccount(
    name: string,
    entityType: string,
    country: string,
    page: number,
