...
import { compareHash, generateCodeAndHash, generateHash } from '@tools/misc';
...
import { UserClient } from '@tools/models/user-clients';
...
export class User extends BaseEntity {
  ...
  getMetadataByClient(uuid: string) {
    return this.userClients?.getItems().find((uc) => uc.client.uuid === uuid)
      ?.metadata;
  }
  ...
  setCurrentClient(uuid: string) {
    this.currentClient =
      this.clients.getItems().find((c) => c.uuid === uuid) ?? null;
  }
  ...
  isMetadataSet(): boolean {
    ...
    if (this.isSubAccount()) {
      // console.log('sub-detail', this.contact.individualMetadata);
      // return !!(this.contact.bankMetadata && this.contact.individualMetadata);
      // Remove bankMetadata as Michail requested. Not sure he's right though
      return (
        !!this.contact.account.individualMetadata ||
        !!this.contact.account.businessMetadata
      );
    }

    const { entityType, bankMetadata, businessMetadata, individualMetadata } =
  ...
  getContactName() {
    return this.contact?.account?.entityType == AccountType.Individual
      ? this.contact?.account?.individualMetadata?.getName()
      : this.contact?.account?.businessMetadata?.getName();
  }
  getFullName(): string {
  ...
  /** Creates a new user with a contact */
  static async withContactFromDto(
