...
@Entity({ customRepository: () => AccountsRepository })
export class Account extends BaseEntity {
  ...
  @OneToMany(
    () => PendingMetaData,
    (pendingMetadata) => pendingMetadata.account,
    {
      mappedBy: 'account',
    },
  )
  ...
  async setBrokers(data: BrokerDto[]) {
    ...
    const savingData = data.map(async (broker) => {
      const newBroker = !broker.uuid
        ? new Broker()
        : oldBrokers.find((oldBroker) => oldBroker.uuid == broker.uuid) ||
          new Broker();
  ...
  async setDirectors(data: DirectorDto[]) {
    ...
    const savingData = data.map(async (director) => {
      const newDirector = !director.uuid
        ? new Director()
        : oldDirectors.find(
            (oldDirector) => oldDirector.uuid == director.uuid,
          ) || new Director();
  ...
  async setShareholders(data: ShareholderDto[]) {
    ...
    const savingData = data.map(async (shareholder) => {
      const newShareholder = !shareholder.uuid
        ? new Shareholder()
        : oldShareholders.find(
            (oldShareholder) => oldShareholder.uuid == shareholder.uuid,
          ) || new Shareholder();
  ...
  async setPendingData(data: PendingMetadataDto[]) {
    ...
    const savingData = data.map(async (metadata) => {
      const newMetadata =
        oldPendingMetas.find(
          (oldMeta) =>
            oldMeta.field == metadata.field && oldMeta.type == metadata.type,
        ) || new PendingMetaData();
