...
import { GenericAbstraction, propertiesToSnakeCase } from '@tools/misc';
...
export class CoreRequester extends GenericAbstraction {
  ...
  protected async getRequest<T = any>(
    ...
    // T é o tipo genérico que será retornado
    ...
    try {
      const response: AxiosResponse<T> = await axios.get<T>(uri, {
        headers,
        params,
      });

      return response.data; // Retorna somente a propriedade data tipada como T
    } catch (error) {
  ...
  protected catchError(error: any): never {
    ...
    if (response && response.data) {
      ...
      Object.entries(errors.error_messages).forEach(
        ([field, fieldMessages]) => {
          const messageArray = fieldMessages as ErrorMessage[]; // Define o tipo explicitamente
          messageArray.forEach((fieldMessage) => {
            console.log(
              `Code: ${fieldMessage.code}, Message: ${fieldMessage.message}`,
            );
            result = [
              ...result,
