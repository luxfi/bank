...
import { CreateBeneficiaryRequest } from '@ports/currency-cloud/requests/beneficiary.request';
...
import {
  RoutingCodeType,
  getRoutingCodeByCountry,
} from 'apps/api/src/currency-cloud/model/routing-code-type.enum';
...
export class CreateBeneficiaryCCUseCase extends CreateBeneficiaryUseCase {
  ...
  async handle(id: string): Promise<BeneficiaryResponse> {
    ...
    if (!response || !response.id) {
      ...
      throw new BadRequestException({
        messages: ['Failed to create beneficiary'],
      });
    }
  ...
  async rollback(beneficiary: Beneficiary) {
    return this.beneficiariesRepository.removeAndFlush(beneficiary);
  }
}
