...
import {
  ...
  PaginationsResponse,
} from './types/transactions.response.type';
...
export class GetTransactionsDomainUseCase extends GetTransactionsUseCase {
  ...
  async handle(
    ...
    if (account || (user_account && user.role !== UserRole.SuperAdmin)) {
      search.$and.push({
        account_id: account ?? user_account,
      });
    }
    ...
    if (currency)
      search.$and.push({
        $or: [{ currency: currency }, { buy_currency: currency }],
      });
    if (beneficiary) search.$and.push({ cdax_beneficiary_id: beneficiary });
    if (reference)
      search.$and.push({
    ...
    if (minAmount) amountSearch.$gte = Number(minAmount);
    if (maxAmount) amountSearch.$lte = Number(maxAmount);
    ...
    if (statusApproval) {
      ...
      const filterStatus = statusApprovals.filter(
        (s) => s !== ETransactionStatus.EXPIRED,
      );
      ...
      if (
        statusApprovals.length === 1 &&
        statusApprovals.includes(ETransactionStatus.EXPIRED)
      ) {
        search.$and.push({
      ...
      if (
        statusApprovals.length === 1 &&
        statusApprovals.includes(ETransactionStatus.PENDING)
      ) {
        search.$and.push({
          payment_date: { $gte: dayjs().format('YYYY-MM-DD') },
        });
