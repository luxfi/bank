...
import { FilterBeneficiariesAccountDTO } from '@tools/models/beneficiaries/dtos/filter-beneficiaries-account.dto';
import { FilterBeneficiariesDTO } from '@tools/models/beneficiaries/dtos/filter-beneficiaries.request.dto';
...
export class BeneficiariesService {
  ...
  async deleteBeneficiary(beneficiary: Beneficiary, owner: User) {
    //FIXME: Log the user who deleted the beneficiary.
  ...
  async approveBeneficiary(beneficiary: Beneficiary, owner: User) {
    //FIXME: Log the user who approved the beneficiary.
    ...
    if (original === beneficiary.isApproved)
      throw new BadRequestException(
        'Could not update the beneficiary in the gateway.',
      );
    let message = '';
    if (beneficiary.isApproved)
    ...
    // console.log(beneficiary.creator.username);
  ...
  async getActiveBeneficiariesByCurrency(user: User, currency: string) {
    return this.beneficiariesRepository.findActiveByUserAndCurrency(
      user,
      currency,
    );
  }

  async getActiveBeneficiariesByAccount(
    accountUuid: string,
    query: FilterBeneficiariesAccountDTO,
  ) {
    return this.beneficiariesRepository.findBeneficiariesByAccount(
      accountUuid,
      query,
    );
  }
  ...
  async hasActiveBeneficiaries(user: User) {
    const activeCount = await this.beneficiariesRepository.countActiveByUser(
      user,
    );
  ...
  async deleteInGateway(owner: User, beneficiary: Beneficiary) {
    if (beneficiary.currencyCloudId) {
      await this.currencyCloudService.deleteBeneficiary(
        beneficiary.currencyCloudId,
        owner.contact?.account?.cloudCurrencyId || null,
        owner,
      );
      beneficiary.currencyCloudId = '';
  ...
  async update(
    ...
    try {
      ...
      const beneficiariesCount =
        await this.beneficiariesRepository.countActiveByUser(updater);
      if (beneficiariesCount > 0) {
        const { beneficiaries: userBeneficiaries } =
          await this.beneficiariesRepository.findActiveByUser(updater);
        const beneficiary = userBeneficiaries.pop();
        if (beneficiary && originalBeneficiary.uuid == beneficiary.uuid) {
          await beneficiary.account.setBankMetadata(
            this.toBankMetadata(beneficiary, beneficiary.account.bankMetadata),
          );
        }
      }
      await this.saveBankDetails(beneficiary, beneficiary.creator);
  ...
  async migrateExistingBeneficiaries(
    user: User,
    gateway: string,
  ): Promise<any> {
    const existingBeneficiaries = (await this.getActiveBeneficiaries(user))
      .beneficiaries;
