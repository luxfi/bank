...
import { isSwift } from '@tools/misc';
import {
  Account,
  AccountType,
  Beneficiary,
...
export class BeneficiaryManager {
  static createFromCreateDto(
    dto: CreateBeneficiaryDto,
    creator: User | null,
  ): Beneficiary {
    if (!creator?.getCurrentClient()?.account) {
      throw new BadRequestException(
        'No account associated with the current user.',
      );
    }
  ...
  static createFromUser(user: User) {
    ...
    if (user.contact.account.entityType == AccountType.Individual) {
      dto.address =
        user.contact.account.individualMetadata?.addressLine1 +
        ', ' +
        user.contact.account.individualMetadata?.addressLine2;
      ...
      if (user.contact.account.businessMetadata?.registeredOffice1_address2) {
        dto.address +=
          ', ' +
          user.contact.account.businessMetadata?.registeredOffice1_address2.trim();
      }
      dto.city = user.contact.account.businessMetadata?.registeredOffice1_city ?? "";
      dto.country =
        user.contact.account.businessMetadata?.countryOfRegistration ?? "";
      dto.postcode =
        user.contact.account.businessMetadata?.registeredOffice1_postcode ?? "";
      dto.state =
        user.contact.account.businessMetadata?.registeredOffice1_state ?? "";
    }
  ...
  static updateFromUser(beneficiary: Beneficiary, user: User) {
    ...
    if (client.account.entityType == AccountType.Individual) {
      beneficiary.address =
        client.account?.individualMetadata?.addressLine1 +
        ', ' +
        client.account?.individualMetadata?.addressLine2;
      ...
      const officeAddress =
        client.account.businessMetadata?.registeredOffice1?.split(',') ?? [];
