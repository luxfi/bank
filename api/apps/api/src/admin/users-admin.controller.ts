...
export class UsersAdminController {
  ...
  async userExists(
    ...
    if (user?.deletedAt) {
      throw new BadRequestException(
        'User was deleted by super admin! You can not use the same email.',
      );
    }
    ...
    const metadata = user?.getMetadataByClient(
      query.clientId ?? req.user.getCurrentClient()?.uuid,
    );
  ...
  async createNewSuperAdmin(
    @RealIP() ip: string,
    @Body() data: AdminCreateSuperUserDto,
  ) {
