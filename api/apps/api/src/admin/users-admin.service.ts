...
import { encrypt, EPaymentProvider, SUPER_CLIENT } from '@tools/misc';
...
import { SelectUsersByClientDto } from '@tools/models/users/dtos/select-users-by-client.dto';
import { AdminCreateSuperUserDto } from '@tools/models/users/dtos/admin-create-super-user.dto';
...
export class UsersAdminService {
    ...
    for (const client of userAdminInClients) {
      ...
      if (adminsInClient.length <= 1) {
        throw new BadRequestException(
          'This user must be an admin in at least one client.',
        );
      }
    }
  ...
  async createLinkedUser(data: LinkedUserDto) {
    ...
    const client = await this.clientsService.findByUuidWithMetadata(
      data.clientId,
    );
    ...
    if (user) {
      if (user.clients.getItems().find((u) => u.uuid === data.clientId)) {
        throw new BadRequestException(
          'User is already registered with this client.',
        );
      }
    } else {
      if (!data.password) {
    ...
    if (data.password) {
      ...
      await this.mailer.send(
        new UserInvitationRoleAlreadyRegistered(data.email, dataEmail),
      );
    }
    return user;
  }
  ...
  async createSelfBeneficiary(user, gateway = 'open_payd') {
    try {
      const hasBeneficiaries =
        await this.beneficiariesService.hasActiveBeneficiaries(user);
      if (!hasBeneficiaries) {
        ...
        await this.beneficiariesService.migrateExistingBeneficiaries(
          user,
          gateway,
        );
      }
    } catch (err) {
      this.logger.error(err.message, (err as Error).stack);
    }
  }
  ...
  async updateCurrencyCloudData(uuid: string, data: CurrencyCloudDataDto) {
    ...
    client.account.gateway = EPaymentProvider.CURRENCY_CLOUND;
  ...
  async restoreUser(uuid: string) {
    ...
    if (!user || !user.archivedAt)
      throw new BadRequestException(
        'The user does not exist or is currently active.',
      );
