...
import { isDemoAccount } from '@tools/misc/utils/clientSetting';
...
export class TwoFaVerificationController {
  ...
  async send(@Body() { provider }: TwoFaCodeDto, @Request() req) {
    const notVerifiedError = new ForbiddenException(
      `couldn't be verified, please try later.`,
    );
  ...
  async check(
    @Request() req,
    @RealIP() ip: string,
    @Body() { provider, code }: TwoFaCodeCheckDto,
    @Headers('User-Agent') userAgent: string,
  ) {
    const notVerifiedError = new ForbiddenException(
      `Code couldn't be verified.`,
    );
    ...
    if (req.user.personatedBy) {
      impersonatedByUser = await this.userService.getUserMeta(
        req.user.personatedBy,
      );
    }

    let verified =
      (process.env.NODE_ENV === 'development' ||
        isDemoAccount(req.user.getCurrentClient().uuid)) &&
      process.env.BYPASS_2FA === code;
      ...
      if (!verified) {
        if (impersonatedByUser) {
          verified = await this.twoFaVerificationService.checkVerification(
            provider === 'email'
              ? (impersonatedByUser.username as string)
              : (impersonatedByUser?.contact?.mobileNumber as string),
            code,
          );
        } else {
          verified = await this.twoFaVerificationService.checkVerification(
