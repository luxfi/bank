...
import { User, UserClient, UserClientsRepository } from '@tools/models';
...
export class AuthService {
  ...
  async getRole(userUUID: string, clientUUID: string) {
    return (
      await this.userClientsRepository.findByUserAndClientUuid(
        userUUID,
        clientUUID,
      )
    )?.metadata.role;
  ...
  async login(user: User, twoFA: boolean, superAdminToken?: string) {
    const payload: PayloadInterface = {
      ...
      role: user.role,
      twoFA,
      ...(user.clients.length && user.getCurrentClient()
        ? { clientUUID: user.getCurrentClient()!.uuid }
        : {}),
