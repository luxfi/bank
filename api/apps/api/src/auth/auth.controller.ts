...
export class AuthController {
  ...
  async login(
    ...
    if (
      ...
      !user.getCurrentClient()?.account?.isApproved
    ) {
    ...
    if (
      ...
      !isDemoAccount(user.getCurrentClient()!.uuid)
    ) {
      await this.twoFaVerificationService.sendVerification(
        dto.username,
        'email',
      );
    }
