...
import { SuccessResponse, SuccessResponseV2 } from '@tools/misc';
import { AdminRoles, UserRole } from '@tools/models';
import { Client } from '@tools/models/clients';
import { CreateClientDto } from '../../../../libs/tools/models/src/clients/dtos/create-client.dto';
import { UpdateClientDto } from '../../../../libs/tools/models/src/clients/dtos/update-client.dto';
...
import {
  GetRiskClientsRequest,
  GetRiskClientsRequestPaginated,
} from './types/get-risk-clients.request.dto';
...
export class ClientController {
  ...
  async getCurrent(@Request() req) {
    const client = await this.clientService.findByUuidWithMetadata(
      req.user.getCurrentClient().uuid,
    );
    ...
    return new SuccessResponse({
      ...
      user: {
        ...
        expectedValueOfTurnover:
          req.user.serialize().contact?.expectedValueOfTurnover ?? '',
        expectedVolumeOfTransactions:
          req.user.serialize().contact?.expectedVolumeOfTransactions ?? '',
  ...
  async listSelect(
    ...
    query: GetClientSelectRequestDto,
  ): Promise<ListClientSelectResponseDto> {
    const { name } = query;

    const { data, pagination } = await this.clientService.getAllClientsSelect(
      name,
    );
  ...
  async getAllRiskAssessments(
    @Query() params: GetRiskClientsRequest,
  ): Promise<ClientSelectRiskPaginatedResponse> {
    const { data, pagination } =
      await this.clientService.getAllRiscAssessmentClients(params);
  ...
  @ApiResponse({
    status: 200,
    description: 'Return paginated risk assessments.',
  })
  async getPaginatedRiskAssessments(
    @Query() params: GetRiskClientsRequestPaginated,
  ): Promise<ClientSelectRiskPaginatedResponse> {
    const { data, pagination } =
      await this.clientService.getPaginatedRiskAssessments(params);
    return new SuccessResponseV2(data, pagination);
  }
  ...
  async list(@Query(new ValidationPipe({ transform: true })) query: any) {
    ...
    const [clients, count] = await this.clientService.getAllClients(
      name,
      entityType,
      country,
      page,
  ...
  async findOne(@Request() req, @Param('uuid') uuid: string) {
    ...
    if (!token) throw new BadRequestException('Client not found');
