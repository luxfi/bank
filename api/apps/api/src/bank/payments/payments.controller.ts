import { ApprovePaymentUseCase, CreatePaymentUseCase } from '@domain/use-cases';
import {
  Body,
  Controller,
  Get,
...
import { GetPurposeCodesUseCase } from '@domain/use-cases/get-purpose-code';
...
export class PaymentsController {
  ...
  getPurposeCodes(
    @Query() purposeCodeRequestDto: PurposeCodeRequestDto,
    @GetUser() user: User,
  ): Promise<PurposeCodeResponseDto> {
  ...
  async create(
    ...
    const { id } = await this.createPaymentUseCase.handle(
      createPaymentDto,
      user,
    );
  ...
  async approve_payment(
    @Param('id') id: string,
    @GetUser() user: User,
  ): Promise<ApprovePaymentResponseDto> {
    const payment = await this.approvePaymentUseCase.handle(
      {
        id,
      },
      user,
  ...
  async reject_payment(
    ...
    const payment = await this.rejectPaymentUseCase.handle(
      {
        id,
        description,
      },
