...
import { UseErrorHandler } from '@tools/misc/filters/god-filter.filter';
import { ManagerRoles, User, UserRole } from '@tools/models';
import { Roles } from '../../auth/roles.decorator';
import {
  CreateBeneficiaryDto,
  GetBeneficiariesDto,
  GetBeneficiariesSelectDto,
  UpdateBeneficiaryDto,
} from './dto/beneficiary.request.dto';
import {
  ApproveAndDisapproveBeneficiaryResponseDto,
  BeneficiaryUpdateResponse,
  GetBeneficiariesPaginatedResponse,
  GetBeneficiariesSelectPaginatedResponse,
  GetBeneficiaryDetailResponse,
...
export class BeneficiariesController {
  ...
  async delete(
    @Param('id') id: string,
    @GetUser() user: User,
  ): Promise<DeleteBeneficiaryResponseDto> {
    await this.archiveBeneficiaryDomainUseCase.handle(id, user);
    return new SuccessResponseV2({
  ...
  async approve(
    @Param('id') id: string,
    @GetUser() user: User,
  ): Promise<ApproveAndDisapproveBeneficiaryResponseDto> {
    const result = await this.approveBeneficiaryDomainUseCase.handle(
      { id },
      user,
    );
  ...
  async disapprove(
    ...
    const result = await this.disapproveBeneficiaryDomainUseCase.handle(
      { id },
      user,
    );
    return new SuccessResponseV2(result);
  ...
  async getCurrent(
    ...
    const { pagination, beneficiaries } =
      await this.getCurrentBeneficiaryUseCase.handle(user);
  ...
  async list(
    @Query() params: GetBeneficiariesDto,
    @GetUser() user: User,
  ): Promise<GetBeneficiariesPaginatedResponse> {
    const { pagination, beneficiaries } =
      await this.getBeneficiariesUseCase.handle(params, user);
    return new SuccessResponseV2(beneficiaries, pagination);
  }
  ...
  async listSelect(
    @Query() params: GetBeneficiariesSelectDto,
    @GetUser() user: User,
  ...
  async detail(
    ...
    const benefiary = await this.getBeneficiaryDetailUseCase.handle(
      { id },
      user,
    );
  ...
  async update(
    ...
    @GetUser() user: User,
  ): Promise<BeneficiaryUpdateResponse> {
    const beneficiary = await this.updateBeneficiaryDomainUseCase.handle(
      id,
      body,
      user,
    );
