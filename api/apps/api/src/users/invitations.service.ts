...
import { generatePassword } from '@tools/misc';
import {
  AccountType,
  Contact,
  Invitation,
...
export class InvitationsService {
  ...
  async requestAccess(data: RequestAccessDto) {
    ...
    await this.requestAccessRepository.persistAndFlush(requestAccess);
  ...
  async create(
    data: InvitationDto,
    creator: User,
    emailType = EmailTypes.InvitationEmail,
  ): Promise<Invitation> {
    ...
    await this.invitationsRepository.persistAndFlush(invitation);
    switch (emailType) {
      ...
      case EmailTypes.AdminInvitationEmail:
        if (data.password)
          await this.mailer.send(
            new AdminInvitationEmail(invitation, secret, data.password),
          );
        break;
      default:
        break;
  ...
  async expire(invitation: Invitation): Promise<Invitation> {
    invitation.expire();
    this.invitationsRepository.persistAndFlush(invitation);
    return invitation;
  }
  ...
  async createInviteUser(data: InviteUserRolesDto, userData: User) {
    ...
    if (user) {
      if (
        user.clients
          .getItems()
          .find((u) => u.uuid === userData.getCurrentClient()?.uuid)
      ) {
    ...
    await this.clientService.putUserOnClient(
      user,
    ...
    const accountName =
      userData.getCurrentClient()?.account?.entityType ===
    ...
    if (password) {
      await this.mailer.send(
        new UserInvitationRole(data.email, { ...dataEmail, password }),
      );
    } else {
      await this.mailer.send(
