...
export class UsersV2Controller {
  ...
  async uploadDocument(
    ...
    const profileImage = await this.documentsService.uploadImage(
      req.user,
      file,
    );

    const user = await this.usersService.updateUsers(req.user.uuid, {
      profileImage,
    });
  ...
  async update(
    ...
    if (
      !user?.clients
        .getItems()
        .find((c) => c.uuid === req.user.getCurrentClient().uuid)
    ) {
      throw new BadRequestException('User is not registered in this client.');
    }
