...
const heroImage = readFileSync(
  join(
    __dirname,
    '../../../templates/emails/registration/verification/hero.gif',
  ),
).toString('base64');
...
const confirmButton = readFileSync(
  join(
    __dirname,
    '../../../templates/emails/registration/verification/confirm-email-btn.gif',
  ),
).toString('base64');
...
export default class VerificationEmail extends BaseEmail<VerificationEmailContext> {
  constructor(to: string, user: User, code: string) {
    super(
      ...
      'emails/registration/verification/verification',
      {
        ...
        code,
        activationUrl: VerificationEmail.getVerificationUrl(
          user.username,
          code,
        ),
      },
  ...
  private static getVerificationUrl(email: string, code: string): string {
    ...
    url.search = `?email=${encodeURIComponent(email)}&code=${encodeURIComponent(
      code,
    )}`;
    return url.toString();
  }
