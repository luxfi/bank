...
import { GetTransactionsResponse, Transaction } from './responses/transactions.response';
...
export class PaymentProviderCurrencyCloud extends CoreRequester {
  ...
  async deleteBeneficiary(id: string, contactId: string | null, user: User): Promise<any | undefined> {
  ...
  async createBeneficiary(data: CreateBeneficiaryRequest, user: User): Promise<CreateBeneficiaryResponse | undefined> {
    try {
      const result = await this.postRequest(
  ...
  async getWalletDetail(params: FundingAccountsRequest, user: User): Promise<FundingAccountsFindResponse> {
    try {
  ...
  async getPurposeCodes(params: PurposeCodeRequest, user: User): Promise<PurposeCodesResponse> {
    try {
  ...
  async getTransactions(params: GetTransactionsRequest, user: User): Promise<GetTransactionsResponse> {
    try {
  ...
  async getConversionDates(conversion_pair: string, user: User): Promise<ReferenceConversionDatesResponse> {
    try {
  ...
  async getCurrencyQuote(data: GetPreviewRequest, user: User): Promise<RatesDetailedResponse> {
  ...
  async getBalances(user: User) {
    try {
      ...
      if (!!response) {
        return {
          balances: response.balances.map((d: any) => ({
            ...d,
            id: d?.currency + d?.id,
          }))
        };
