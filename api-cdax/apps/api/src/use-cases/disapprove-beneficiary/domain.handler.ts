...
export class DisapproveBeneficiaryDomainUseCase {
    ...
    async handle(query: DisapproveBeneficiaryRequest, user: User): Promise<DisapproveBeneficiaryResponse> {
        ...
        if (!beneficiary.gatewayId)
            throw new BadRequestException({messages: ['Beneficiary not found on gateway.']});
    ...
    private async createOnCurrencyCloud(beneficiary: Beneficiary) {
        ...
        const cloudCurrencyId = beneficiary.account.gatewayId ?? beneficiary.account.cloudCurrencyId;
