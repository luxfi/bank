...
export class CreatePaymentCCUseCase extends CreatePaymentUseCase {
    ...
    async handle(createPaymentDto: CreatePaymentsRequest, user: User): Promise<CreatePaymentsResponse> {
        ...
        if (status_approval === TransactionStatusApproval.Done) {
            ...
            if (!result)
                throw new BadRequestException({messages: ['Payment could not be created on currencycloud']});
            ...
            console.log('Payment created with status approval pending', paymentTransaction);
        }
