...
export class HmacIFXGuard implements CanActivate {
    ...
    canActivate(context: ExecutionContext): boolean {
        ...
        if (!userAgent || !timestamp || !receivedSignature || !body)
            throw new HttpException('Missing required headers or body', HttpStatus.BAD_REQUEST);

        const version = userAgent.split('/')[1];
        if (!version)
            throw new HttpException('Invalid User-Agent format', HttpStatus.BAD_REQUEST);
